	...
	public static final List<NVBytes> getCompleteRow (final String tableName, final byte[] pk, 
		...
		try {
			...
			if ( table.exists(getter) ) {
				...
				List<NVBytes> nvs = new ArrayList<NVBytes>(r.listCells().size());
				for (Cell cell : r.listCells()) {
					NVBytes nv = new NVBytes(CellUtil.cloneFamily(cell),CellUtil.cloneQualifier(cell), CellUtil.cloneValue(cell));
					nvs.add(nv);
				}
				return nvs;
			}
