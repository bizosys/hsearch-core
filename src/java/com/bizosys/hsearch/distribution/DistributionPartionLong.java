...
public final class DistributionPartionLong {

	protected long[] distributes(Collection<Long> inputs, int parts) {
		
		long[] output = new long[parts - 1];
		long[] ranges = new long[]{Long.MIN_VALUE, Long.MAX_VALUE};
		GravityCenter gc = distribute(inputs, ranges);
		output[0] = gc.avgValue;
		System.out.println(output[0]);
		ranges = new long[]{Long.MIN_VALUE, gc.avgValue};
		for ( int i=1; i< (parts/2); i++) {
			GravityCenter mingc = distribute(inputs, ranges);
			output[i] = mingc.avgValue;
			ranges = new long[]{Long.MIN_VALUE, mingc.avgValue};
		}

		ranges = new long[]{gc.avgValue, Long.MAX_VALUE};
		for ( int i=parts/2; i< parts - 1; i++) {
			GravityCenter maxgc = distribute(inputs, ranges);
			output[i] = maxgc.avgValue;
			ranges = new long[]{maxgc.avgValue, Long.MAX_VALUE};
		}
	...
	private GravityCenter distribute(Collection<Long> inputs, long[] ranges) {
	...
	private void getAverage(Collection<Long> inputs, GravityCenter wts, long[] ranges) {
	...
	private void getWeights(Collection<Long> inputs, GravityCenter weights, long[] ranges) {
		int left=0, right=0;
		long avgValue = weights.avgValue;
		for (long i : inputs) {
			if ( i < ranges[0] || i > ranges[1] ) continue;
			if (i >= avgValue) right++;
			else left++;
