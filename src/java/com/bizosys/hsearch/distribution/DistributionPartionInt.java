...
public final class DistributionPartionInt {

	protected int[] distributes(Collection<Integer> inputs, int parts) {
		
		int[] output = new int[parts - 1];
		int[] ranges = new int[]{Integer.MIN_VALUE, Integer.MAX_VALUE};
		GravityCenter gc = distribute(inputs, ranges);
		output[0] = gc.avgValue;
		System.out.println(output[0]);
		ranges = new int[]{Integer.MIN_VALUE, gc.avgValue};
		for ( int i=1; i< (parts/2); i++) {
			GravityCenter mingc = distribute(inputs, ranges);
			output[i] = mingc.avgValue;
			ranges = new int[]{Integer.MIN_VALUE, mingc.avgValue};
		}

		ranges = new int[]{gc.avgValue, Integer.MAX_VALUE};
		for ( int i=parts/2; i< parts - 1; i++) {
			GravityCenter maxgc = distribute(inputs, ranges);
			output[i] = maxgc.avgValue;
			ranges = new int[]{maxgc.avgValue, Integer.MAX_VALUE};
		}
	...
	private GravityCenter distribute(Collection<Integer> inputs, int[] ranges) {
	...
	private void getAverage(Collection<Integer> inputs, GravityCenter wts, int[] ranges) {
	...
	private void getWeights(Collection<Integer> inputs, GravityCenter weights, int[] ranges) {
		int left=0, right=0;
		int avgValue = weights.avgValue;
		for (int i : inputs) {
			if ( i < ranges[0] || i > ranges[1] ) continue;
			if (i >= avgValue) right++;
			else left++;
